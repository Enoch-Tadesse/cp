...
ll dfs(ll curr, ll con, ll k, vll& cats, umap<ll, vll>& adj, ll par) {
    if (cats[curr] == 1)
        con += 1;
    else
        con = 0;
    if (con > k) return 0;
    if (adj[curr].size() == 1 && curr != 0) return 1LL;
    ll counter = 0;
    for (ll nei : adj[curr]) {
		if (nei == par) continue;
        counter += dfs(nei, con, k, cats, adj, curr);
    }
    return counter;
}
